...
def transform_distance_stats(T_list, T_mean):
    trans = np.stack([T[:3, 3] for T in T_list], axis=0)
    t_mean = T_mean[:3, 3]
    d = np.linalg.norm(trans - t_mean[None, :], axis=1)
    trans_std = np.std(trans, axis=0, ddof=1) if len(trans) > 1 else np.zeros(3)
    dist_std = float(np.std(d, ddof=1)) if len(d) > 1 else 0.0
    ...
    rot_std_deg = float(np.std(angs, ddof=1)) if len(angs) > 1 else 0.0

    return trans_std, dist_std, rot_std_deg
...
def main():
    ...
    ap.add_argument("--rot-std-threshold-deg", type=float, default=2.0)
    ap.add_argument("--trans-std-threshold-m", type=float, default=0.01)
    args = ap.parse_args()
    ...
    with open(args.timestamps_csv, "r", encoding="utf-8-sig") as f:
    ...
    for tid, Ts in samples.items():
        if len(Ts) < args.min_samples:
            print(f"[WARN] skip tag {tid}: only {len(Ts)} samples < min_samples={args.min_samples}")
            continue

        T_mean = mean_transform(Ts)
        trans_std_xyz, dist_std, rot_std_deg = transform_distance_stats(Ts, T_mean)

        low_confidence = (
            len(Ts) < args.min_samples or
            rot_std_deg > args.rot_std_threshold_deg or
            np.max(trans_std_xyz) > args.trans_std_threshold_m
        )
        ...
        result["tags"][str(tid)] = {
            ...
            "translation_std_xyz_m": trans_std_xyz.tolist(),
            "translation_distance_std_m": dist_std,
        ...
        print(
            f"tag {tid}: n={len(Ts)}, "
            f"trans_std_xyz={trans_std_xyz}, "
            f"rot_std_deg={rot_std_deg:.3f}, "
